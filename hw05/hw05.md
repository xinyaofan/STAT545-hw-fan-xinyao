...
sl_long <- sl_drop %>%
  ...
  select(year, artist_hotttnesss, artist_familiarity, duration,top_artist) %>%
  gather(key = "Measure", value = "values",artist_familiarity:artist_hotttnesss)

sl_long%>%
  filter(top_artist!="Other")%>%
  ggplot(aes(x = year, y = values, colour=top_artist)) + geom_point(alpha=1) + facet_wrap(~Measure,scales = "free")+scale_fill_brewer(palette = "Dark2")
```
