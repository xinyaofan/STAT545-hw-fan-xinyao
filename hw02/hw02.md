...
There are three classes in "gapminder": tbl\_df, tbl and data.frame.
...
As for the lifExp variable(quantative variable), we'd better use histogram to find its distribution.
...
Now, we explore a subset of data.
...
The result is not correct, becasue they omit several lines. When we use logic symbol "==",the first country will compare with "Rwanda", the second one will compare with"Afghanistan", the third one will compare with "Rwanda", the fourth will compare with"Afghanistan".So, it lost half of the data. The correct one is the following:
...
For this homework, it is easy to use a single syntax, such as get the subset of data, plot a historgram or boxplot(even we have nott studied class). However, when I combined these syntax together to analyze a complex problem, things become difficult.
