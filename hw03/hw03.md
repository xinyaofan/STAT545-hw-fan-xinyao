...
gapminder%>%
  group_by(continent)%>%
  mutate(ccolor = (min(gdpPercap) == gdpPercap | max(gdpPercap) == gdpPercap))%>%
  ggplot(aes(x=year,y=gdpPercap))+geom_point(aes(color=ccolor))+facet_wrap(~continent)
```
...
```
    ...
    ##  3 Afghanistan      Asia  1962  31.997 10267083  853.1007         FALSE
    ##  4 Afghanistan      Asia  1967  34.020 11537966  836.1971         FALSE
...
From graph above, it shows that "blue" point is the maximum GDP per capita for each continent, and triangle is the minimum GDP per capita for each continent.<br> Actually, we can know which continent and when gets the largest gdpPercap using the following code:
...
<<<<<<< HEAD
So, it is Asia achieved the largest gdpPercap in 1957. **Q2** Look at the spread of GDP per capita within the continents.
=======
So, it is Asia achieved the largest gdpPercap in 1957. <br>


**Q2** Look at the spread of GDP per capita within the continents.
>>>>>>> e3496b1153273b32d26a9d651e0cefb580e97ea9
...
![](hw03_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-13-1.png)
